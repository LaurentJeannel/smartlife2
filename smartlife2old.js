...
for(i = 0; i < longueur; i++) {
       eval( k + i + ' = '+ "new TuyAPI({id: objet["+i+"]['id'],key: objet["+i+"]['key'],issueRefreshOnConnect: true})");
      ff=objet[i]["name"]
...
l = 'asyncCall'
for(j=0;j<longueur;j++){eval( l + j + '().catch(console.error).then(() => console.log(""));')}
}

if(result ===undefined){console.log('rien'); return false}

if(data.verifdomo=="vérif"){
        eval(m+result+".get({schema: true}).catch(JarvisIASpeech('objet hors ligne')).then(dataa => {console.log(dataa.dps['1']) ;console.log(dataa.dps['20']); console.log(dataa);if(dataa.dps['1']==false){JarvisIASpeech('objet off')};if(dataa.dps['1']==true){JarvisIASpeech('objet on')};if(dataa.dps['20']==false){JarvisIASpeech('objet off')};if(dataa.dps['20']==true){JarvisIASpeech('objet on')};} )");
    return false
}
